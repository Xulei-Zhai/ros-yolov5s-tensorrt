...
static uint8_t* img_buffer_device = nullptr;  // 初始值为nullptr，表示指针当前没有指向任何有效的内存地址。

// 这两个静态变量在代码中的目的是为了在图像预处理过程中存储图像数据的主机和设备内存地址。
//它们的使用可以在多次调用图像预处理函数时重复利用已分配的内存，而不需要每次都重新分配内存，以提高效率。
