...
static ILayer* convBlock(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int g, std::string lname) {
  ...
  assert(sig);
  auto ew = network->addElementWise(*bn1->getOutput(0), *sig->getOutput(0), ElementWiseOperation::kPROD);//相当于将sigmoid的结果乘上x,来作为整体的激活函数结果
  assert(ew);
  return ew;
}
...
static std::vector<std::vector<float>> getAnchors(std::map<std::string, Weights>& weightMap, std::string lname) {
  std::vector<std::vector<float>> anchors;  // 创建一个二维向量anchors，用于存储锚点信息。
  Weights wts = weightMap[lname + ".anchor_grid"]; // 从权重映射中获取与lname + ".anchor_grid"相关联的权重，将其存储在wts变量中。
  int anchor_len = kNumAnchor * 2;  // 定义一个整数变量anchor_len，表示每个锚点的长度，即锚点数乘以2（因为每个锚点由两个坐标值组成）。
  for (int i = 0; i < wts.count / anchor_len; i++) {  //遍历锚点权重数组，每次迭代处理一个锚点。
    auto *p = (const float*)wts.values + i * anchor_len; // 在每次迭代中，根据锚点长度计算当前锚点的起始指针位置，并将其强制转换为const float*类型。
    std::vector<float> anchor(p, p + anchor_len); // 使用起始指针和锚点长度创建一个std::vector<float>，表示当前锚点。
    anchors.push_back(anchor); // 使用起始指针和锚点长度创建一个std::vector<float>，表示当前锚点。
  }
  return anchors;
}
...
static IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, std::string lname, std::vector<IConvolutionLayer*> dets, bool is_segmentation = false) {
  auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1"); // 获取名为"YoloLayer_TRT"、版本号为"1"的插件创建器。
  auto anchors = getAnchors(weightMap, lname); // 调用getAnchors函数，从权重映射中获取与给定名称相关联的锚点信息，并将其存储在anchors变量中。
  PluginField plugin_fields[2];  // 创建一个包含两个PluginField的数组，用于定义插件的字段信息。
  // 定义一个整数数组netinfo，包含了网络的相关信息，如类别数量、输入图像的宽度和高度、最大输出边界框数目以及分割标识。
  int netinfo[5] = {kNumClass, kInputW, kInputH, kMaxNumOutputBbox, (int)is_segmentation};  
  plugin_fields[0].data = netinfo;  // 将netinfo的指针赋值给plugin_fields[0].data，
  plugin_fields[0].length = 5; // 并设置length字段。
  plugin_fields[0].name = "netinfo";  // 并设置name字段。
  plugin_fields[0].type = PluginFieldType::kFLOAT32;  //并设置type字段。
  ...
  Weights strides = weightMap[lname + ".strides"];
   // 将 strides.values 的指针转换为 const float* 类型，并使用 p 来引用所指向的内存位置，以便后续操作可以在该内存位置上进行。
