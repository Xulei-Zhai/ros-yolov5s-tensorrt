...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      cv::Rect r = get_rect( img_batch[i], res[j].bbox);
    ...
    // cv::circle( img_batch[i],{50,50},5, cv::Scalar(0x27, 0xC1, 0x36), 2);
      cv::rectangle( img_batch[i], r, cv::Scalar(0x27, 0xC1, 0x36), 2);
      cv::putText( img_batch[i], std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
    }
  }
}
